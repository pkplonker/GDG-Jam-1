...
public class PlayerMovementState2ElectricBoogaloo : IState, IMovement
{
    ...
    public void FixedTick()
    {
        ...
        transform.RotateAround(transform.position, transform.up, movement.y < 0.1f && movement.y > -0.1f ? movement.x * statemachine.MovementStats.rotateSpeed : movement.y * movement.x * statemachine.MovementStats.rotateSpeed);
        ...
        if (rb.velocity.magnitude > statemachine.MovementStats.maxSpeed) //if horizontal velocity is greater than set speed
        {
            Vector3 limitedVel = rb.velocity.normalized * statemachine.MovementStats.maxSpeed; //sets velocity ot be max speed
